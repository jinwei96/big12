				...
				if(msg != null){
					...
					switch (msg.getMessageType()){
						...
						case BaseMessage.CLIENT_TO_SERVER_CHAT:
						{
							...
							String senderAddr = msg0.getSenderAddr();
							String msgTxt = msg0.getMessage();
                            QQClientChatSingleUI singleUI = ui.allSingleChat.get(senderAddr);
                            if (singleUI == null){
                                singleUI = new QQClientChatSingleUI(senderAddr,this);
                                ui.allSingleChat.put(senderAddr,singleUI);
                            }
                            singleUI.setVisible(true);
                            singleUI.updateHistory(senderAddr,msgTxt);
                            break ;
						}
