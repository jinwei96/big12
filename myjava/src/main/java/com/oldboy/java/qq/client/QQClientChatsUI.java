...
public class QQClientChatsUI extends JFrame implements ActionListener {
	...
	//所有私聊窗口
	public Map<String,QQClientChatSingleUI> allSingleChat = new HashMap<>();
		...
		lstFriends.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				if(e.getClickCount() == 2){
					...
					QQClientChatSingleUI singleUI = allSingleChat.get(recvAddr);
					if (singleUI == null){
						singleUI = new QQClientChatSingleUI(recvAddr,commThread);
						allSingleChat.put(recvAddr,singleUI);
					}
