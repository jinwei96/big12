...
public class TestFilter {
    ...
    public void scanData7() throws Exception {
        ...
        //初始化行过滤器,使用二进制比较器，小于等于row005
        RowFilter filter1 = new RowFilter(CompareFilter.CompareOp.LESS_OR_EQUAL,
                new BinaryComparator(Bytes.toBytes("row005")));

        //初始化行过滤器,使用二进制比较器，大于等于row095
        RowFilter filter2 = new RowFilter(CompareFilter.CompareOp.GREATER_OR_EQUAL,
    ...
    public void scanData8() throws Exception {
        ...
        //初始化行过滤器,使用二进制比较器，小于等于row005
        FamilyFilter filter1 = new FamilyFilter(CompareFilter.CompareOp.EQUAL,
                new RegexStringComparator("f1"));

        //初始化行过滤器,使用二进制比较器，大于等于row095
        QualifierFilter filter2 = new QualifierFilter(CompareFilter.CompareOp.EQUAL,
                new RegexStringComparator("name"));


        FilterList fl = new FilterList( filter1,filter2);
